...
const (
	...
	GreeterService_Chat_FullMethodName     = "/greet.v1.GreeterService/Chat"
)
...
//
// GreeterService greets people and echoes chat messages.
...
func (c *greeterServiceClient) Chat(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[ChatRequest, ChatResponse], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &GreeterService_ServiceDesc.Streams[0], GreeterService_Chat_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
...
type GreeterService_ChatClient = grpc.BidiStreamingClient[ChatRequest, ChatResponse]
...
func (UnimplementedGreeterServiceServer) Chat(grpc.BidiStreamingServer[ChatRequest, ChatResponse]) error {
...
func _GreeterService_Chat_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(GreeterServiceServer).Chat(&grpc.GenericServerStream[ChatRequest, ChatResponse]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type GreeterService_ChatServer = grpc.BidiStreamingServer[ChatRequest, ChatResponse]
