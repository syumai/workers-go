...
// from Go via syscall/js. It is used to exercise the inbound socket code
// paths (Listener, Socket, the handleConnect binding) without a real
// Workers runtime.
...
// openedInfo builds an `opened`-shaped resolved Promise with the given
// addresses. Empty strings are omitted (left undefined), matching how the
// real runtime reports an unknown address.
func openedInfo(localAddr, remoteAddr string) js.Value {
	info := jsutil.NewObject()
	if localAddr != "" {
		info.Set("localAddress", localAddr)
	}
	if remoteAddr != "" {
		info.Set("remoteAddress", remoteAddr)
	}
	return jsutil.PromiseClass.Call("resolve", info)
}
